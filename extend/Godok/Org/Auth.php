...
final class Auth
{
     ...
     * 行为验证
     */
    public static function actionBegin()
    {
        ...
        if (!is_array($userRule)) {
            self::error('对不起！您无权进行该操作.', \think\Request::instance()->root() . '/operate/Login/index');
        }
	...
	private static function fresh() {
		...
		if ( empty( self::user() ) ) {
			...
			if ( $group && ! empty( $group['rules'] ) ) {
				$user = [ 'rules'      => array_unique( explode( ',', $group['rules'] ) ),
				          'groupids'   => [ 3 ],
				          '_freshtime' => time()
				];
				self::user( $user );
			...
			Config::get( 'auth.auth_type' ) == '1' || '1' == cache('is_auth_group_cache:'.$group_id)
			|| (
			...
			//登录用户
			$data = $db->name( Config::get( 'auth.table_user' ) )->field( 'id,username,nickname,avatar,phone,deleted,status' )->where( 'id', self::user( 'id' ) )->find();
			...
			//清除权限组更新缓存
			cache('is_auth_group_cache:'.$group_id,'0');
			Auth::user( $user );
    ...
    public static function getGroups($uid = null, $onlyid = false)
    {
        ...
        $db = Db::connect();
        $user_groups = $db->view(Config::get('auth.table_group_relation'), 'uid,group_id')
    ...
    {
        $identifying = Session::get('identifying');;
        $config = \think\Loader::model('Database', 'service')->dispatch($identifying);
