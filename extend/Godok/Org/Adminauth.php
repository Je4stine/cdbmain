...
final class Adminauth
{
    ...
    public static function actionBegin($call)
    {
        ...
        if (!is_array($userRule)) {
            self::error('对不起！您无权进行该操作.',\think\Request::instance()->root().'/admin/Login/index');
        }
        ...
        //根据请求找出匹配中的规则
        $query =  Db::name( Config::get('auth.admin_rule') )->field('`condition`,`id`')->where('status',1);
        if (!empty($module)) {
    ...
    private static function fresh(){ 
        if ( empty(self::user()) ) {
            //游客登录
            $group = Db::name( Config::get('auth.admin_group') )->field('id,rules')->where('id',3)->find();
            ...
            if( empty(self::user('id'))) {
                ...
                //登录用户
                $data=Db::name( Config::get('auth.admin_user') )->field('id,username,nickname,avatar,phone,deleted,status')->where('id',self::user('id'))->find();
    ...
    public static function menu($where = false , $pid = 0)
    {
        ...
        $query =  Db::name( Config::get('auth.admin_rule') );
        if( !in_array('all', $ids)) {
    ...
    public static function getGroups($uid = null, $onlyid = false)
    {
        ...
        $user_groups = Db::view( Config::get('auth.admin_group_relation'), 'uid')
            ->view( Config::get('auth.admin_group') , 'status,rules,id,listorder,title', Config::get('auth.admin_group').'.id='.Config::get('auth.admin_group_relation').'.group_id')
    ...
    public static function check($action, $uid = null) {
        ...
        foreach($action as $val) {
            ...
            $route = explode('/',$val);
            $query = Db::name( Config::get('auth.admin_rule') )->field('id')->where('status',1);
            $act = array_pop($route);
            if( $i = strpos($act,'?') ) {
