...
class Cache {
    ...
    static public function set($name, $value, $expired = 0) {
        ...
        if (self::check()) {
            file_put_contents(self::$cachepath . $name, $data);
            exec('sudo chmod  777 ' . self::$cachepath . $name);
            return true;
        }
    }
    ...
    static public function put($line, $filename = '') {
        ...
        if (self::check()) {
            if (file_exists(self::$cachepath . $filename)) {
                exec('sudo chmod -R 777' . self::$cachepath . $filename);
            }
            return file_put_contents(self::$cachepath . $filename, '[' . date('Y/m/d H:i:s') . "] {$line}\n", FILE_APPEND);
        }
    }
